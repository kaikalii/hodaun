...
fn main() {
    ...
    let a = SineWave::new(FREQ).amplify(0.5).take(DUR);
    let b = SineWave::new(FREQ).amplify(0.1).take(DUR);
    let c = SineWave::new(FREQ).amplify(0.9).take(DUR);
    let d = SineWave::new(FREQ).amplify(0.3).take(DUR);
